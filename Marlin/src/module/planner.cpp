...
bool Planner::_populate_block(block_t * const block, bool split_move,
    ...
    if (millimeters)
      ...
      block->millimeters = SQRT(
          ...
          sq(delta_mm[X_AXIS]) + sq(delta_mm[Y_HEAD]) + sq(delta_mm[Z_HEAD])
            ...
            #endif
          #endif
        #elif defined(FOAMCUTTER_XY_IJ) // return the largest distance moove from either X/Y or I/J plane
  ...
  block->step_event_count = _MAX(block->steps[A_AXIS], block->steps[B_AXIS], block->steps[C_AXIS]
    #if NON_E_AXES > 3
      , block->steps[I_AXIS]
      #if NON_E_AXES > 4
        , block->steps[J_AXIS]
  ...
  if (block->step_event_count < MIN_STEPS_PER_SEGMENT) return false;
    ...
    if (block->steps[X_AXIS] || block->steps[Y_AXIS] || block->steps[Z_AXIS]
      ...
      powerManager.power_on(); // TODO (DerAndere) Test for NON_E_AXES > 3
    }
  #endif
