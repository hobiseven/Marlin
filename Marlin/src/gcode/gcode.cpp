...
static constexpr bool ar_init[NUM_AXIS] = AXIS_RELATIVE_MODES;
uint8_t GcodeSuite::axis_relative = (
    (ar_init[X_AXIS] ? _BV(REL_X) : 0)
  | (ar_init[Y_AXIS] ? _BV(REL_Y) : 0)
  | (ar_init[Z_AXIS] ? _BV(REL_Z) : 0)
  #if NON_E_AXES > 3
    | (ar_init[I_AXIS] ? _BV(REL_I) : 0)
    #if NON_E_AXES > 4
      | (ar_init[J_AXIS] ? _BV(REL_J) : 0)
      #if NON_E_AXES > 5
        | (ar_init[K_AXIS] ? _BV(REL_K) : 0)
      #endif
    #endif
  #endif
...
void GcodeSuite::get_destination_from_command() {
  ...
  LOOP_NUM_AXIS(i) {
    ...
    if (recovery.enabled && IS_SD_PRINTING() && seen[E_AXIS] && (seen[X_AXIS] || seen[Y_AXIS])) // TODO : Likely not working in case of additional axes moove
      recovery.save();
  #endif
...
void GcodeSuite::process_parsed_command(const bool no_ok/*=false*/) {
  ...
  switch (parser.command_letter) {
    ...
    case 'M': switch (parser.codenum) {
        ...
        #if (HAS_CUTTER || (FAN_COUNT > 0))
          case 3: M106(); break;                                  // M106: Fan On > Turn on hot wire
          case 5: M107(); break;                                  // M107: Fan Off > Turn off hot wire
        #endif
      #else
        #if HAS_CUTTER
